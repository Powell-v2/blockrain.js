  ...
  $.widget('aerolab.blockrain', {
    ...
    ___takeCare(rows) {
      ...
      for (let innerIdx = 0; innerIdx < rowsRange; ++innerIdx) {
        ...
        const landingTargetY = this.___vansSwapping
          ? towardsSaucer
          : towardsVan
        ...
        const extractIndividualChunks = () => {
          ...
          for (let chunkIdx = 0; chunkIdx < this._BLOCK_WIDTH; chunkIdx++) {
            ...
            const landingTargetX = this.___vansSwapping
              ? (this._BLOCK_WIDTH / 2 - chunkIdx - 0.5) * actualChunkWidth
              : (
                actualRowWidth +
            ...
            this.___generatedRules.push(yAxisRuleName, xAxisRuleName)
            ...
            if (animationDuration > longestAnimationDuration) {
              ...
              $elementWithLongestAnimationDuration = $blownChunkWrapper
            }
      ...
      if (this.___vansSwapping) {
        this.___initiateSaucerScene(
          longestAnimationDuration,
          $blownLinesWrapper,
          $elementWithLongestAnimationDuration,
        )
      }
      else {
        this.___initiateVansSwappingScene(
          $blownLinesWrapper,
          $elementWithLongestAnimationDuration,
        )
      }
    },
    ___initiateVansSwappingScene(
      $blownLinesWrapper,
      $elementWithLongestAnimationDuration,
    ) {
      const onAnimationEnd = () => {
        ...
        this.___deleteStyleRules()
        $blownLinesWrapper.remove()

        $elementWithLongestAnimationDuration.off(`animationend`, onAnimationEnd)
      }

      $elementWithLongestAnimationDuration.on(`animationend`, onAnimationEnd)

      $blownLinesWrapper.appendTo(`.blockrain-game-holder`)
    },
    ___initiateSaucerScene(
      longestAnimationDuration,
      $blownLinesWrapper,
      $elementWithLongestAnimationDuration,
    ) {
